/**
 * This is startupOrchestrator.ts
...
import { getDrives } from './fs.wrapped';
...
async function executeStartupSequence(options: StartupOptions): Promise<void> {
  ...
  try {
    ...
    if (!options.skipDriveDiscovery) {
      ...
      // Register with GPS for timeout tracking
      const DRIVE_OP_ID = 'startup-drives';
      import('$lib/core/GPS').then(({ gps }) => {
        gps.startProcess(DRIVE_OP_ID, 'folder-load', 'GLOBAL', 10_000); // 10s timeout
        
        // Fire and forget - don't block startup
        getDrives()
          .then(async (drives) => {
            gps.endProcess(DRIVE_OP_ID);
